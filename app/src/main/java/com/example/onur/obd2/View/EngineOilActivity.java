...
import com.example.onur.obd2.Model.Sensors.EngineOilTemperature;
...
public class EngineOilActivity extends AppCompatActivity {
            ...
            public void run()
            {
                runOnUiThread(new Runnable()
                {
                    ...
                    public void run()
                    {
                        ...
                        engineOilTemperature = new EngineOilTemperature(getData());

                        _time.setText(Integer.toString(StartActivity.counter));
                        _value.setText(engineOilTemperature.getValue() + " " + engineOilTemperature.getUnit());
                        _status.setText(getStatus(engineOilTemperature.getValue()));
                    }
                });
            }
        }, 1000, 1000);
    ...
    private String getStatus(double amount)
    {
        ...
        double part = (EngineOilTemperature.minValue + EngineOilTemperature.maxValue) / 3;

        if(EngineOilTemperature.minValue <= amount && amount < part)
        {
            return "RISKY";
        }
